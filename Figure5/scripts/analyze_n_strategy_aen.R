...
emm_df <- as.data.frame(emm1) %>%
  ...
  mutate(n_lab = paste0("n=", k))
write.csv(emm_df, "Figure5b_data1.csv")
...
write.csv(jt_by_env, "Figure5b_data2.csv")
...
emm_df <- as.data.frame(emm1) %>%
  ...
  left_join(count_cell, by = c("env_group","N_strategy")) %>%
  mutate(n_lab = paste0("n=", k))
write.csv(emm_df, "Figure5d_data1.csv")
...
print(jt_by_env)
write.csv(jt_by_env, "Figure5d_data2.csv")
# ---- position p labels above the highest CI in each env ----
y_top <- emm_df %>%
  group_by(env_group) %>%
  summarise(ymax = max(pct_upr, na.rm = TRUE), .groups = "drop")
...
emm_df <- as.data.frame(emm1) %>%
  ...
  left_join(count_cell, by = c("env_group","N_strategy")) %>%
  mutate(n_lab = paste0("n=", k))
write.csv(emm_df, "Figure5f_data1.csv")
# ---- Omnibus per env_group: H0 (within env): all N_strategy means equal ----
jt0 <- as.data.frame(test(pairs(emm1), joint = TRUE))
...
print(jt_by_env)
write.csv(jt_by_env, "Figure5f_data2.csv")
# ---- position p labels above the highest CI in each env ----
y_top <- emm_df %>%
  group_by(env_group) %>%
  summarise(ymax = max(pct_upr, na.rm = TRUE), .groups = "drop")
