...
write.csv(emm_df, "Figure5a_data1.csv")
...
write.csv(jt_by_env, "Figure5a_data2.csv")
...
print(wilcox_res2)
write.csv(wilcox_res2, "Figure5g_data.csv", row.names = FALSE)
yield_summary2 <- summarise_yield_box(dA, "Figure5g", "USA_China")
write.csv(yield_summary2, "Figure5g_yield_summary.csv", row.names = FALSE)
...
emm_df <- as.data.frame(emm1) %>%
  ...
  left_join(count_cell, by = c("env_group","N_strategy")) %>%
  mutate(n_lab = paste0("n=", k))
write.csv(emm_df, "Figure5c_data1.csv")
...
print(jt_by_env)
write.csv(jt_by_env, "Figure5c_data2.csv")
# ---- position p labels above the highest CI in each env ----
y_top <- emm_df %>%
  group_by(env_group) %>%
  summarise(ymax = max(pct_upr, na.rm = TRUE), .groups = "drop")
...
print(wilcox_res2)
write.csv(wilcox_res2, "Figure5h_china_yield_wilcox.csv", row.names = FALSE)
yield_summary2 <- summarise_yield_box(dA, "Figure5h", "China")
write.csv(yield_summary2, "Figure5h_china_yield_summary.csv", row.names = FALSE)
...
emm_df <- as.data.frame(emm1) %>%
  ...
  left_join(count_cell, by = c("env_group","N_strategy")) %>%
  mutate(n_lab = paste0("n=", k))
write.csv(emm_df, "Figure5e_data1.csv")
# ---- Omnibus per env_group: H0 (within env): all N_strategy means equal ----
jt0 <- as.data.frame(test(pairs(emm1), joint = TRUE))
...
print(jt_by_env)
write.csv(jt_by_env, "Figure5e_data2.csv")
# ---- position p labels above the highest CI in each env ----
y_top <- emm_df %>%
  group_by(env_group) %>%
  summarise(ymax = max(pct_upr, na.rm = TRUE), .groups = "drop")
...
print(wilcox_res2)
write.csv(wilcox_res2, "Figure5i_USA_yield_wilcox.csv", row.names = FALSE)
yield_summary2 <- summarise_yield_box(dA, "Figure5i", "USA")
write.csv(yield_summary2, "Figure5i_USA_yield_summary.csv", row.names = FALSE)
