...
write.csv(dA, out_file("region_n_dataset.csv"))
...
write.csv(emm_df, out_file("lnrr.csv"))
...
write.csv(pw1, out_file("tukey_p.csv"))
# =========================
...
ggsave(
  out_file("forest_pct_by_env_group_density_tukeyp_flip.pdf"),
...
print(wilcox_res2)
write.csv(wilcox_res2, out_file("yield_wilcox.csv"), row.names = FALSE)
...
ggsave(out_file("box_yield_envXdensity_wilcox_letters.pdf"), p_box_letters2,
...
fit_df <- d_plot %>%
  ...
  ungroup()
write.csv(fit_df, out_file("linear_lnrr.csv"))
# -------------------------
...
ggsave(out_file("lines_edh_effect.pdf"), p_line,
...
fit_df <- d_plot %>%
  ...
  ungroup()
write.csv(fit_df, out_file("linear_aen.csv"))
# -------------------------
# 2) annotation positions (stack 3 lines of text)
# -------------------------
x0 <- min(d_plot$EDH_sum, na.rm = TRUE) + 0.02 * diff(range(d_plot$EDH_sum, na.rm = TRUE))
...
ggsave(out_file("lines_edh_aen.pdf"), p_line,
       width = 54, height = 48, units = "mm", dpi = 300)
...
fit_df <- d_plot %>%
  ...
  ungroup()
write.csv(fit_df, out_file("linear_yield.csv"))
# -------------------------
# 2) annotation positions (stack 3 lines of text)
# -------------------------
x0 <- min(d_plot$EDH_sum, na.rm = TRUE) + 0.02 * diff(range(d_plot$EDH_sum, na.rm = TRUE))
...
ggsave(out_file("lines_edh_mean_t.pdf"), p_line,
       width = 54, height = 48, units = "mm", dpi = 300)
