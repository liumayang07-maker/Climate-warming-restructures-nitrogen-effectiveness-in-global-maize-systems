...
out_dir <- file.path(root_dir, "output_data", "aen")
...
## =========================================================
## 3) Preprocess: build d0 and d_env (keep finite AEN, IDs)
## =========================================================
d0 <- dat %>%
  mutate(
    AEN = .data[[col_AEN]],
...
## -------------------------
m1 <- nlme::lme(
  fixed  = AEN ~ env_group,     # Normal as reference
  random = ~ 1 | reference_ID/site_year_ID,
  data   = d_env,
  method = "REML",
  na.action = na.omit,
...
write.csv(pw_both, file.path(out_dir, "Pairwise_env_none_and_tukey_AEN.csv"), row.names = FALSE)
...
x_max_data <- max(res_plot$pct_upr, 30, na.rm = TRUE)

x_lim_low  <- x_min_data
x_lim_high <- x_max_data + 1   # room for "n=" (tune 6~14)
...
p <- ggplot(res_plot, aes(y = group)) +
  ...
  labs(x = "Agronomic efficiency of N (kg grain / kg N)")

print(p)

ggsave(
  file.path(out_dir, "Forest_overall_plus_env_AEN.pdf"),
...
##    AEN ~ 0 + env_group:N_rate_cluster
## =========================================================
cat("\n[Model] Cell means: AEN ~ 0 + env_group:N_rate_cluster (unweighted lme)\n")

mB <- nlme::lme(
  fixed  = AEN ~ 0 + env_group:N_rate_cluster,
  random = ~ 1 | reference_ID/site_year_ID,
  data   = d1,
  method = "REML",
  na.action = na.omit,
...
write.csv(pw_tab_none,  file.path(out_dir, "Pairwise_env_within_Ncluster_none_AEN.csv"),  row.names = FALSE)
write.csv(pw_tab_tukey, file.path(out_dir, "Pairwise_env_within_Ncluster_Tukey_AEN.csv"), row.names = FALSE)
...
y_max <- max(pw_plot$y + bracket_headroom * y_pad, 50, na.rm = TRUE)
...
ggsave(
  file.path(out_dir, "Forest_env_by_Ncluster_AEN.pdf"),
...
dAIC <- d %>% select(AEN, env_group, N_rate, Nc, reference_ID, site_year_ID)
...
m2_ML <- lme(
  AEN ~ env_group * poly(Nc, 2, raw = TRUE),
  random = ~ 1 | reference_ID/site_year_ID,
  data = dAIC, method = "ML",
  control = lmeControl(opt = "optim")
)
...
write.csv(ftab_best, file.path(out_dir, "best_model_ANOVA_AEN.csv"))
...
## Build RHS-only formula for model.matrix (avoid 'AEN not found')
...
ggsave(
  file.path(out_dir, "DoseResponse_env_curves_curveOnly_AEN.pdf"),
