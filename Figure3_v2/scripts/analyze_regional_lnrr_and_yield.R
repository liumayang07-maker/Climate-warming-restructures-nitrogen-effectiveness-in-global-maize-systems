...
# =========================================================
m_lin <- nlme::lme(
  fixed  = lnRR ~ env_group * continent + Nc,
...
m_ns3 <- nlme::lme(
  fixed  = lnRR ~ env_group * continent + ns(N_rate, df = 3),
  random = ~ 1 | reference_ID/site_year_ID,
  data   = d_trim,
  method = "ML",
  na.action = na.omit,
...
m_ns4 <- nlme::lme(
  fixed  = lnRR ~ env_group * continent + ns(N_rate, df = 4),
  random = ~ 1 | reference_ID/site_year_ID,
  data   = d_trim,
  method = "ML",
  na.action = na.omit,
...
emm_df <- as.data.frame(emm0) %>%
  mutate(
    pct     = (exp(emmean)   - 1) * 100,
    pct_lwr = (exp(lower.CL) - 1) * 100,
    pct_upr = (exp(upper.CL) - 1) * 100,
...
cfg <- list(
  ...
  # Public-release note
  br_base_gap = 10.0,  # Public-release note
  br_step     = 18.0,  # Public-release note
  br_tick     = 5,  # Public-release note
  ...
  # Public-release note
  p_text_dy   = 8,  # Public-release note
  p_size      = 2.0,
...
p_forest_continent <- ggplot(emm_df, aes(x = continent, y = pct, color = env_group)) +
  ...
  geom_text(
    data = emm_df,
    aes(x = continent, y = 20, label = n_lab, group = env_group, angle = cfg$n_angle),
    position = position_dodge(width = cfg$dodge_w),
    color = "black",
    size = cfg$n_size,
    vjust = cfg$n_vjust,
...
# =========================================================
box_stats <- d_trim %>%
  filter(!is.na(mean_t)) %>%  # Public-release note
  group_by(continent, env_group) %>%
  summarise(
    n       = sum(!is.na(mean_t)),
    ...
    q25     = quantile(mean_t, probs = 0.25, na.rm = TRUE, type = 7),
    q75     = quantile(mean_t, probs = 0.75, na.rm = TRUE, type = 7),
    p05     = quantile(mean_t, probs = 0.05, na.rm = TRUE, type = 7),
    p95     = quantile(mean_t, probs = 0.95, na.rm = TRUE, type = 7),
    .groups = "drop"
  ) %>%
  mutate(
    continent = factor(continent, levels = levels(d_trim$continent)),
...
# =========================================================
box_stats <- d_trim %>%
  filter(!is.na(N_rate)) %>%  # Public-release note
  group_by(continent, env_group) %>%
  summarise(
    n       = sum(!is.na(N_rate)),
    mean    = mean(N_rate, na.rm = TRUE),
    sd      = sd(N_rate, na.rm = TRUE),
    median  = median(N_rate, na.rm = TRUE),
    q25     = quantile(N_rate, probs = 0.25, na.rm = TRUE, type = 7),
    q75     = quantile(N_rate, probs = 0.75, na.rm = TRUE, type = 7),
    p05     = quantile(N_rate, probs = 0.05, na.rm = TRUE, type = 7),
    p95     = quantile(N_rate, probs = 0.95, na.rm = TRUE, type = 7),
    .groups = "drop"
  ) %>%
  mutate(
    continent = factor(continent, levels = levels(d_trim$continent)),
...
# =========================================================
box_stats <- d_trim %>%
  filter(!is.na(mean_c)) %>%  # Public-release note
  group_by(continent, env_group) %>%
  summarise(
    n       = sum(!is.na(mean_c)),
    mean    = mean(mean_c, na.rm = TRUE),
    sd      = sd(mean_c, na.rm = TRUE),
    median  = median(mean_c, na.rm = TRUE),
    q25     = quantile(mean_c, probs = 0.25, na.rm = TRUE, type = 7),
    q75     = quantile(mean_c, probs = 0.75, na.rm = TRUE, type = 7),
    p05     = quantile(mean_c, probs = 0.05, na.rm = TRUE, type = 7),
    p95     = quantile(mean_c, probs = 0.95, na.rm = TRUE, type = 7),
    .groups = "drop"
  ) %>%
  mutate(
    continent = factor(continent, levels = levels(d_trim$continent)),
...
# =========================================================
m0 <- nlme::lme(
  fixed  = lnRR ~ env_group * country + ns(N_rate, df = 4),
  random = ~ 1 | reference_ID/site_year_ID,
  data   = d_trim,
  method = "REML",
  na.action = na.omit,
...
p_forest_country <- ggplot(emm_df, aes(x = country, y = pct, color = env_group)) +
  ...
  geom_text(
    data = emm_df,
    aes(x = country, y = 20, label = n_lab, group = env_group, angle = cfg$n_angle),
    position = position_dodge(width = cfg$dodge_w),
    color = "black",
    size = cfg$n_size,
    vjust = cfg$n_vjust,
