...
d_trim <- d0 %>%
  ...
  filter(!is.na(env_group), !is.na(N_strategy), is.finite(AEN)) %>%
  mutate(
...
# ---- within-env trimming of AEN (5%-95%) ----
dA <- d_trim %>%
  group_by(env_group) %>%
  mutate(
...
# =========================================================
m0 <- nlme::lme(
  fixed  = AEN ~ env_group * N_strategy + Nc,
...
emm_df <- as.data.frame(emm1) %>%
  ...
  mutate(n_lab = paste0("n=", k))
write.csv(emm_df, "Extended Data Figure 2b_data1.csv")
...
write.csv(jt_by_env, "Extended Data Figure 2b_data2.csv")
...
p_forest_env_strategy_pct <- ggplot(emm_df, aes(x = env_group, y = pct, color = N_strategy)) +
  ...
  geom_text(
    data = emm_df,
    aes(x = env_group, y = 5, label = n_lab, group = N_strategy),
    position = position_dodge(width = cfg$dodge_w),
    color = "black",
    size = cfg$n_size,
    vjust = cfg$n_vjust,
...
ggsave(
  "USA_China_AEN_forest_OmnibusP_N_strategy.pdf",
...
emm_df <- as.data.frame(emm1) %>%
  ...
  left_join(count_cell, by = c("env_group","N_strategy")) %>%
  mutate(n_lab = paste0("n=", k))
write.csv(emm_df, "Extended Data Figure 2d_data1.csv")
...
print(jt_by_env)
write.csv(jt_by_env, "Extended Data Figure 2d_data2.csv")
# ---- position p labels above the highest CI in each env ----
y_top <- emm_df %>%
  group_by(env_group) %>%
  summarise(ymax = max(pct_upr, na.rm = TRUE), .groups = "drop")
...
ggsave(
  "China_AEN_forest_OmnibusP_N_strategy.pdf",
  p_forest_env_strategy_pct,
  width = 76, height = 38, units = "mm", dpi = 300
)
...
emm_df <- as.data.frame(emm1) %>%
  ...
  left_join(count_cell, by = c("env_group","N_strategy")) %>%
  mutate(n_lab = paste0("n=", k))
write.csv(emm_df, "Extended Data Figure 2f_data1.csv")
# ---- Omnibus per env_group: H0 (within env): all N_strategy means equal ----
jt0 <- as.data.frame(test(pairs(emm1), joint = TRUE))
...
print(jt_by_env)
write.csv(jt_by_env, "Extended Data Figure 2f_data2.csv")
# ---- position p labels above the highest CI in each env ----
y_top <- emm_df %>%
  group_by(env_group) %>%
  summarise(ymax = max(pct_upr, na.rm = TRUE), .groups = "drop")
...
ggsave(
  "USA_AEN_forest_OmnibusP_N_strategy.pdf",
  p_forest_env_strategy_pct,
  width = 76, height = 38, units = "mm", dpi = 300
)
